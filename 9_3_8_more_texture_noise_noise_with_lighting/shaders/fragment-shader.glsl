...
void main() {
  ...
  // float noiseSample = fbm(gridSeed, 10, 0.5, 2.0);
  // noiseSample += 0.5;
  // // noiseSample = remap(noiseSample, -1.0, 1.0, 0.0, 1.0);
  ...
  //turbulence with lighting effect
  float noiseSample = turbulenceFbm(gridSeed, 4, 0.5, 2.0);
  colour = vec3(noiseSample);
  
  //generate a normal map by sampling in 4 directions
  vec3 pixel = vec3(0.5 / resolution, 0.0); // res will be something like 1000x900 or whatever, so we need to divide by that to get a single pixel offset in uv space, 0.5 because we want to sample in the middle of the pixel i think...
  float sRight = turbulenceFbm(gridSeed + pixel.xzz, 4, 0.5, 2.0);
  float sLeft = turbulenceFbm(gridSeed - pixel.xzz, 4, 0.5, 2.0);
  float sTop = turbulenceFbm(gridSeed + pixel.zyz, 4, 0.5, 2.0);
  float sBottom = turbulenceFbm(gridSeed - pixel.zyz, 4, 0.5, 2.0);

  vec3 normal = normalize(vec3(sRight - sLeft, sTop - sBottom, 0.0025));
  
  //hemi lighting
  vec3 skyColour = vec3(0.0, 0.3, 0.6);
  vec3 groundColour = vec3(0.6, 0.3, 0.1);

  vec3 hemiLighting = mix(groundColour, skyColour, remap(normal.y, -1.0, 1.0, 0.0, 1.0));
  ...
  //specular lighting
  vec3 r = normalize(reflect(-diffuseLightDirection, normal));
  float phongValue = max(0.0, dot(normalize(vec3(0.0, 0.0, 1.0)), r));
  phongValue = pow(phongValue, 32.0);
  vec3 specularLighting = vec3(0.0);
  specularLighting += phongValue;
  
  //final lighting mix
  vec3 lighting = hemiLighting * 0.125 + diffuseLight * 0.5;
  ...
  colour = baseColour * lighting + specularLighting;

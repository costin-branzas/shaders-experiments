...
void main() {
  ...
  float dIntersectionCirclesWithBox = max(dUnionCircles, dBox1);

  float dSubtraction = max(dUnionCircles, -dBox1);

  // float d = dUnion;
  // float d = dIntersectionCirclesWithBox;
  float d = dSubtraction;
    
  
  // color = mix(red, color, step(0.0, d)); // suffers from aliasing - jagged edges
  color = mix(red * 0.5, color, smoothstep(-1.0, 1.0, d)); //use smoothstep to anti alias and mutiply red with 0.5 to make the ENTIRE circle darker
  color = mix(red, color, smoothstep(-5.0, 0.0, d)); //then reapply bright red all the way until -20 inside the circle trasitioning to the dark color defined above

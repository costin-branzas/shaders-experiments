...
void main() {
  ...
  vec3 baseColour = v_color;

  // vec3 normal = normalize(v_normal); //this is the normal taken from the vertex shader, but because we warped the geometry in the vertex shader, the normal is no longer correct (still basically as it was received in the vertex shader before the warping)
  // so, another method for calculating the normal using dFdx and dFdy:
  vec3 normal = normalize(
      cross( dFdx(v_position.xyz), dFdy(v_position.xyz)) ); //.xyz seems pointless, works without it as well...

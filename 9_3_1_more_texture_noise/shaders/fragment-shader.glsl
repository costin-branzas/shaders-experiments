...
void main() {
  ...
  float gridSize = 8.0;
  vec3 gridSeed = vec3(v_uv.x * gridSize, v_uv.y * gridSize, time * 0.5);
  
  float noiseSample = fbm(gridSeed, 2, 0.5, 2.0); // 3 octaves, persistance 0.5 (how quickly amplitude drops off), lacunarity 2.0 (how quickly frequency increases)
